      ...
      <div className="bg-white rounded-2xl border border-slate-200/90 shadow-2xs p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 border-t border-slate-100 text-xs">
          ...
          <div className="p-3 rounded-xl bg-rose-50/60 border border-rose-200/60 font-semibold">
            <span className="text-rose-700 block">Total Gastos Esporádicos del Periodo</span>
        ...
        <div className="flex items-center gap-2 pt-1 text-xs">
          <span className="font-semibold text-slate-500">Filtrar:</span>
          <button
          ...
          filteredList.map((tx) => {
            ...
            return (
              <div
                key={tx.id}
                id={`tx-card-${tx.id}`}
                className="bg-white rounded-2xl border border-slate-200/90 hover:border-slate-300 transition-all shadow-2xs p-4 flex flex-wrap items-center justify-between gap-3 text-slate-700"
              >
                      ...
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${
                          isIncome
                            ...
                            : 'bg-rose-100 text-rose-800 border-rose-150'
                        }`}
                      >
                        {tx.tag}
                      </span>
                      {tx.isScheduled && (
                        <span className="text-[10px] font-bold px-2 py-0.2 rounded bg-indigo-50 text-indigo-705 border border-indigo-200">
                          Programado
                        </span>
                      )}
                    </div>
