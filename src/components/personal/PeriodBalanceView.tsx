...
import { AppData, DebtItem, FreeBalanceAllocation, PeriodSelection, SavingsProgram } from '../../types';
        ...
        <div className="bg-white rounded-2xl border border-slate-200/90 shadow-2xs p-5 space-y-4">
          ...
          <div className="space-y-2.5 text-xs font-semibold">
            ...
            <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-200/60 text-xs">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-white border border-slate-200 flex items-center justify-center text-slate-750">
            ...
            {summary.sporadicIncomes.length > 0 ? (
              summary.sporadicIncomes.map((inc) => (
                <div
                  key={inc.id}
                  className="flex items-center justify-between p-3 rounded-xl bg-emerald-50/50 border border-emerald-200/60 text-xs font-semibold"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 font-semibold border border-emerald-150">
                          {inc.tag}
                        </span>
                      </div>
                  ...
                  <span className="font-extrabold text-emerald-705 font-mono">
                    +{formatCurrency(inc.amount, currencyCode, currencySymbol)}
                  </span>
                </div>
              ))
            ) : (
              <div className="p-3 rounded-xl border border-dashed border-slate-200 text-center text-xs text-slate-400 font-normal">
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <div className="flex items-center gap-1.5">
                      ...
                      {exp.isSkipped ? (
                        <span className="text-[10px] bg-amber-100 text-amber-800 font-bold px-1.5 py-0.2 rounded border border-amber-150">
                          Omitido
                        </span>
                      ) : (
                        <span className="text-[10px] bg-slate-100 text-slate-650 px-1.5 py-0.2 rounded">
                          {exp.tag}
                        </span>
                      )}
                    </div>
                    <span className="block text-[10px] text-slate-500 font-normal">Gasto fijo recurrente</span>
                  </div>
                </div>

                <div className="text-right">
                  <span className={`font-extrabold font-mono ${exp.isSkipped ? 'line-through text-slate-450' : 'text-slate-900'}`}>
            ...
            {summary.debtDues.map((debtDue) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <div className="flex items-center gap-1.5 font-bold">
                        <span className={`${debtDue.isSkipped ? 'line-through text-slate-500' : 'text-slate-900'}`}>
                          {debtDue.title}
                        </span>
                        {debtDue.isSkipped ? (
                          <span className="text-[10px] px-1.5 py-0.2 rounded bg-amber-100 text-amber-800 font-bold border border-amber-150">
                            Cuota Omitida
                          </span>
                        ) : (
                          <span className="text-[10px] px-1.5 py-0.2 rounded bg-rose-100 text-rose-800 font-semibold border border-rose-150">
                            {debtDue.tag}
                          </span>
                        )}
                      </div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`font-extrabold font-mono ${debtDue.isSkipped ? 'line-through text-slate-400' : 'text-rose-750'}`}>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <div className="flex items-center gap-1.5 font-bold">
                        <span className={`${savDue.isSkipped ? 'line-through text-slate-500' : 'text-slate-900'}`}>
                          {savDue.name}
                        </span>
                        {savDue.isSkipped ? (
                          <span className="text-[10px] px-1.5 py-0.2 rounded bg-amber-100 text-amber-800 font-bold border border-amber-150">
                          ...
                          <span className="text-[10px] px-1.5 py-0.2 rounded bg-teal-100 text-teal-805 font-semibold border border-teal-150">
                            {savDue.tag}
                          </span>
                        )}
                      </div>
                  ...
                  <div className="flex items-center gap-2 text-xs font-semibold">
                    <span className={`font-extrabold font-mono ${savDue.isSkipped ? 'line-through text-slate-400' : 'text-teal-750'}`}>
            ...
            {summary.sporadicExpenses.map((exp) => (
              ...
              >
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <div className="flex items-center gap-1.5 font-bold">
                      <span className={`${exp.isSkipped ? 'line-through text-slate-500' : 'text-slate-900'}`}>
                        {exp.title}
                      </span>
                      <span className="text-[10px] px-1.5 py-0.2 rounded bg-amber-100 text-amber-800 font-semibold border border-amber-150">
                        {exp.tag}
                      </span>
                      {exp.isSkipped && (
                    ...
                    <span className="block text-[10px] text-slate-505 font-normal">{exp.date}</span>
                  </div>
                </div>
