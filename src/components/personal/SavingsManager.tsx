...
import { formatCurrency, getPeriodKey } from '../../utils/formatters';
...
interface SavingsManagerProps {
  ...
  skippedObligations?: Record<string, string[]>;
}
...
export const SavingsManager: React.FC<SavingsManagerProps> = ({
  ...
  const currentPeriodKey = getPeriodKey(period.year, period.month, period.quincena);
  const skippedList = skippedObligations?.[currentPeriodKey] || [];
  const skippedSavingsCount = savings.filter(s => skippedList.includes(s.id)).length;
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-white rounded-2xl border border-slate-200/90 shadow-2xs p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 border-t border-slate-100 text-xs">
          <div className="p-3 rounded-xl bg-teal-50/60 border border-teal-200/60 text-slate-700">
          ...
          <div className="p-3 rounded-xl bg-indigo-50/60 border border-indigo-200/60 text-indigo-900 font-medium">
            <span className="text-indigo-700 block">Programas Activos</span>
      ...
      {skippedSavingsCount > 0 && (
        <div className="p-4 bg-amber-50 border border-amber-250 text-amber-900 rounded-2xl text-xs font-semibold flex items-center gap-3 animate-in slide-in-from-top-2 duration-200">
          <div className="w-8 h-8 rounded-full bg-amber-100 flex items-center justify-center shrink-0 text-base">
            ⚠️
          </div>
          <div>
            <p className="font-bold">¡Alerta de Retraso de Ahorro!</p>
            <p className="text-amber-700 mt-0.5 leading-relaxed font-normal">
              Tienes {skippedSavingsCount} {skippedSavingsCount === 1 ? 'meta' : 'metas'} de ahorro marcadas como "Omitidas" (Skipped) en esta quincena. Omitir aportes programados postergará el cumplimiento de tus metas financieras y la fecha límite original estimada.
            </p>
          </div>
        </div>
      )}
              ...
              >
                <div className="p-5 space-y-4 text-slate-700">
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3 text-xs">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {skippedList.includes(item.id) && (
                          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 border border-amber-250">
                            ⚠️ Omitido esta quincena
                          </span>
                        )}
                      ...
                      {item.notes && (
                        <p className="text-xs text-slate-500 italic font-semibold">{item.notes}</p>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="px-3.5 py-1.5 text-[10px] font-black text-white bg-teal-650 hover:bg-teal-700 rounded-xl shadow-xs transition-colors cursor-pointer flex items-center gap-1.5"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 text-slate-400 hover:text-rose-650 hover:bg-rose-550 rounded-lg transition-colors cursor-pointer"
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs bg-slate-50/80 p-3 rounded-xl border border-slate-200/60 font-semibold">
                    <div>
                      <span className="text-slate-400 block text-[10px]">Aporte por Periodo</span>
                    ...
                    <div>
                      <span className="text-slate-400 block text-[10px]">Total Ahorrado Actual</span>
                    ...
                    <div>
                      <span className="text-slate-400 block text-[10px]">Meta Deseada</span>
                      <span className="font-bold text-slate-800">
                        {item.targetAmount
                          ? formatCurrency(item.targetAmount, currencyCode, currencySymbol)
                          : 'Ahorro continuo'}
                    ...
                    <div>
                      <span className="text-slate-400 block text-[10px]">Total Aportes Registrados</span>
                      <span className="font-bold text-slate-800">
