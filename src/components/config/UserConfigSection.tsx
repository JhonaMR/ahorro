...
import { getUserQRs, uploadUserQR, deleteUserQR } from '../../utils/storage';
...
export const UserConfigSection: React.FC<UserConfigSectionProps> = ({
  ...
  const [qrType, setQrType] = useState('ahorros'); // ahorros | corriente
  ...
  const handleUploadQrSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setQrError(null);
    setQrSuccess(null);

    if (!qrName.trim() || !qrBank.trim() || !qrFile) {
      setQrError('Por favor completa todos los campos y selecciona una imagen QR.');
      return;
    }
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        setQrSuccess('Código QR cargado con éxito.');
        setQrName('');
        setQrBank('');
        setQrFile(null);
        ...
        setQrError(err.message || 'Error al cargar el código QR.');
      }
    };
    reader.onerror = () => {
      setQrError('Error al leer el archivo de imagen.');
    };
  ...
  const handleDeleteQr = async (id: string) => {
    ...
    setQrSuccess(null);
    try {
      await deleteUserQR(id);
      setQrSuccess('Código QR eliminado con éxito.');
      fetchQrs();
    } catch (err: any) {
      setQrError(err.message || 'Error al eliminar el código QR.');
    }
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-200">
      ...
      {/* 1.5. MIS CODIGOS QR DE PAGO */}
      <div className="bg-white rounded-3xl border border-slate-200/90 p-6 sm:p-8 shadow-xs space-y-6 animate-in fade-in duration-300">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-black text-slate-900">
                Mis Códigos QR de Pago
              </h2>
              <p className="text-xs text-slate-500">
                Sube hasta 2 códigos QR (Nequi, Bancolombia, etc.) para que otros integrantes del grupo los escaneen al saldar cuentas.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Form to add QR */}
          {qrs.length < 2 ? (
            <form onSubmit={handleUploadQrSubmit} className="space-y-4 text-xs font-semibold text-slate-700">
              <h3 className="text-sm font-bold text-slate-800">Agregar Nuevo QR</h3>
              
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block uppercase tracking-wider mb-1">Nombre Personalizado</label>
                  <input
                    type="text"
                    required
                    placeholder="ej. Nequi Luis, Cuenta Personal"
                    value={qrName}
                ...
                <div>
                  <label className="block uppercase tracking-wider mb-1">Banco / Entidad</label>
                  <input
                    type="text"
                    required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block uppercase tracking-wider mb-1">Tipo de Cuenta</label>
                  <select
                    value={qrType}
                ...
                <div>
                  <label className="block uppercase tracking-wider mb-1">Imagen del Código QR</label>
                  <input
                    id="qr-file-input"
                    type="file"
                    required
                    ...
                    className="w-full px-2 py-1.5 bg-slate-50 border border-slate-300 rounded-xl text-slate-900 font-medium focus:ring-2 focus:ring-indigo-500 outline-none"
                  />
                </div>
              </div>
            ...
            <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl flex flex-col items-center justify-center text-center space-y-2">
              ...
              <p className="text-xs font-bold text-slate-700">Ya has alcanzado el límite de 2 códigos QR</p>
              <p className="text-[11px] text-slate-500">Elimina uno de tus códigos QR existentes para poder cargar otro.</p>
            </div>
          )}

          {/* List of QRs */}
          <div className="space-y-4">
            <h3 className="text-sm font-bold text-slate-800">Códigos QR Activos</h3>
            {loadingQrs ? (
              <p className="text-xs text-slate-500">Cargando códigos...</p>
            ) : qrs.length === 0 ? (
              <p className="text-xs text-slate-500">No tienes ningún código QR registrado aún.</p>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {qrs.map((qr) => (
                  <div key={qr.id} className="p-4 bg-slate-50 border border-slate-200 rounded-2xl flex flex-col justify-between space-y-3 relative group">
                    <div className="space-y-1.5">
                      <div className="flex items-center gap-2">
                        <Building2 className="w-4 h-4 text-slate-400" />
                        <span className="text-xs font-bold text-slate-900">{qr.bankName}</span>
                      </div>
                      <p className="text-[11px] text-slate-500 font-semibold">{qr.accountName} • <span className="capitalize">{qr.accountType}</span></p>
                    </div>

                    <div className="w-full aspect-square bg-white rounded-xl border border-slate-200 p-2 flex items-center justify-center overflow-hidden">
                      <img src={qr.qrImageUrl} alt="QR Code" className="max-w-full max-h-full object-contain" />
                    </div>

                    <button
                      type="button"
                      onClick={() => handleDeleteQr(qr.id)}
                      className="w-full py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-600 hover:text-rose-700 border border-rose-200 rounded-xl text-[10px] font-bold flex items-center justify-center gap-1.5 cursor-pointer transition"
                    >
