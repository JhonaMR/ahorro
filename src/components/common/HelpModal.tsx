    ...
    switch (activeTab) {
      ...
      case 'shared_finances':
        return {
          ...
          general: 'Ideal para gestionar gastos conjuntos (alquiler compartido, viajes grupales, compras de víveres en pareja) sin mezclar tus cuentas personales por completo. Permite registrar quién pagó qué y calcular las deudas netas consolidando los saldos.',
          loadingData: [
            'Crear Grupo: Crea un espacio compartido con un nombre (ej. "Hogar", "amigos") y añade a los participantes.',
            ...
            'Saldar Cuentas (Sub-pestaña): Permite saldar deudas pendientes con otros miembros del grupo. Al presionar "Saldar", se despliega un desglose detallado con checkboxes y el código QR de cobro del acreedor para escanear y transferir instantáneamente.',
            'Simulador Gastos (Sub-pestaña): Módulo colaborativo informativo donde cada miembro declara sus ingresos (que puede ocultar tras asteriscos para mayor privacidad) y gastos fijos comunes (Arriendo, Internet, etc.) para calcular la proporción exacta de aporte sugerido.'
          ],
          distribution: [
            ...
            'Saldar con QRs: Cada usuario puede subir hasta 2 imágenes QR desde su panel de configuración para que se muestren a los deudores al saldar cuentas.',
            'Ingresos Ocultos en Simulador: Si seleccionas "Ocultar mi ingreso", los demás verán "$ *******" en la tabla, pero el sistema calculará correctamente las proporciones en el servidor.'
          ],
          reports: [
            'Saldar deudas neto: Resumen automático consolidado del balance neto que debes o te deben.',
            'Aporte Sugerido Proporcional: Reporte en el Simulador que sugiere cuánto debe poner cada persona según sus ingresos para cubrir los gastos compartidos del grupo.'
          ]
        };
      case 'calendar':
        return {
      ...
      case 'config':
        return {
          ...
          loadingData: [
            ...
            'Colchón de Seguridad: Configura el saldo de base inicial del colchón (ahorro histórico de emergencia) y la fecha de corte (año, mes y quincena de inicio) a partir de la cual empezará a acumularse históricamente en base a tu slider de saldo libre.',
            'QRs de Cobro: Sube hasta 2 imágenes de códigos QR de pago de tus cuentas (especificando nombre, banco y tipo de cuenta de ahorros/corriente) para facilitar que otros miembros del grupo te transfieran dinero en el módulo de Saldar Cuentas.',
            'Respaldos (Backup): Permite exportar toda tu base de datos a un archivo JSON o importar un respaldo previo.'
          ],
          reports: [
            'Estado de conexión de la Base de Datos.',
            'Historial del colchón acumulado (se calcula a partir de la fecha de inicio configurada).'
          ]
        };
      default:
        return {
