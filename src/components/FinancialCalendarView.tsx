...
export const FinancialCalendarView: React.FC<FinancialCalendarViewProps> = ({
  ...
  const compileEventsForMonth = React.useCallback((year: number, month: number) => {
    ...
    // C) Cuotas de Deudas pagadas en el mes
    ...
    // D) Depósitos de Ahorro personal registrados en el mes
    ...
    // 3. 🟡 COMPARTIDOS POR ETIQUETAR / CLASIFICAR (Amarillo)
    ...
    // 4. 🔵 LO DE FAMILIA / FINANZAS COMPARTIDAS (Azul)
  ...
  const selectedDayMetrics = useMemo(() => {
    ...
    return { income, expense, pending, family, net: income - expense };
  }, [selectedDayEvents]);
  ...
  return (
    <div id="financial-calendar-module" className="space-y-5 animate-in fade-in duration-200 pb-8">
      ...
      <div className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200/90 shadow-xs flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-indigo-50 text-indigo-700 border border-indigo-200/70 capitalize">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {/* View Toggle (Grid vs List vs Year) */}
          <div className="flex items-center p-1 bg-slate-100/90 rounded-xl border border-slate-200/90 shadow-2xs">
            <button
              ...
              title="Vista Calendario en Cuadrícula"
            >
          ...
          {/* Expand / Collapse Detail Section Button - Always visible and functional */}
          <button
            type="button"
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 border shadow-2xs active:scale-95 ${
              isDetailExpanded
                ...
                : 'bg-white text-indigo-700 border-indigo-300 hover:bg-indigo-50 shadow-slate-200'
            }`}
            title={
              isDetailExpanded
                ? 'Contraer panel de detalle (ver calendario a pantalla completa)'
                : 'Expandir panel de detalle del día'
            }
          >
            {isDetailExpanded ? (
              <>
                <PanelRightClose className="w-3.5 h-3.5 text-white shrink-0" />
                <span className="inline sm:hidden">Contraer</span>
                <span className="hidden sm:inline">Contraer Detalle</span>
              </>
            ) : (
              <>
                <PanelRightOpen className="w-3.5 h-3.5 text-indigo-600 shrink-0" />
                <span className="inline sm:hidden">Expandir</span>
      ...
      {/* 4 Colored Category Metric Cards for the Month */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        >
          ...
          <div className="text-[11px] text-emerald-700/80 font-medium mt-0.5">
            {allEvents.filter((e) => e.category === 'income').length} movimientos registrados
          </div>
        </div>
        ...
        >
          ...
          <div className="text-[11px] text-rose-700/80 font-medium mt-0.5">
            {allEvents.filter((e) => e.category === 'expense').length} salidas personales
          </div>
        </div>

        {/* 🟡 Amarillo: Por Etiquetar / Clasificar */}
        <div
        ...
        >
          ...
          <div className="text-[11px] text-amber-700/80 font-medium mt-0.5">
            {allEvents.filter((e) => e.category === 'pending_shared').length} pendientes / divididos
          </div>
        </div>
        ...
        >
          ...
          <div className="text-[11px] text-blue-700/80 font-medium mt-0.5">
            {allEvents.filter((e) => e.category === 'family').length} registros familiares
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-3 rounded-2xl border border-slate-200/90 shadow-2xs">
        ...
        <div className="relative w-full sm:w-64">
          ...
          <input
            ...
            placeholder="Buscar por título, tag o persona..."
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
        {/* Calendar Grid or List View (8 Cols on Desktop when expanded, 12 Cols when collapsed) */}
        <div
          className={`${
        ...
        >
                ...
                {calendarCells.map((cell) => {
                  ...
                  return (
                    <div
                      ...
                      title={isHoliday && cell.isCurrentMonth ? 'Día Festivo' : undefined}
                    ...
                    >
                      ...
                      <div className="space-y-1 my-1">
                        ...
                        {cell.incomeTotal > 0 && (
                          <div
                            className="text-[10px] font-bold bg-emerald-50 text-emerald-800 border border-emerald-200/80 px-1 py-0.5 rounded flex items-center justify-between leading-tight truncate"
                            title={`Ingresos: ${formatCurrency(cell.incomeTotal, currencyCode, currencySymbol)}`}
                          >
                        ...
                        {cell.expenseTotal > 0 && (
                          <div
                            className="text-[10px] font-bold bg-rose-50 text-rose-800 border border-rose-200/80 px-1 py-0.5 rounded flex items-center justify-between leading-tight truncate"
                            title={`Gastos: ${formatCurrency(cell.expenseTotal, currencyCode, currencySymbol)}`}
                          >
                        ...
                        {/* 🟡 Por Clasificar badge */}
                        {cell.pendingTotal > 0 && (
                          <div
                            className="text-[10px] font-extrabold bg-amber-50 text-amber-800 border border-amber-300 px-1 py-0.5 rounded flex items-center justify-between leading-tight truncate"
                            title={`Por clasificar / divididos: ${formatCurrency(cell.pendingTotal, currencyCode, currencySymbol)}`}
                          >
                        ...
                        {cell.familyTotal > 0 && (
                          <div
                            className="text-[10px] font-bold bg-blue-50 text-blue-800 border border-blue-200/80 px-1 py-0.5 rounded flex items-center justify-between leading-tight truncate"
                            title={`Finanzas familiares: ${formatCurrency(cell.familyTotal, currencyCode, currencySymbol)}`}
                          >
            ...
            <div className="p-4 sm:p-5 flex-1 flex flex-col space-y-4">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="p-3 bg-white border border-slate-200/80 rounded-xl text-center shadow-3xs">
                  <span className="text-slate-500 font-semibold text-xs block">Ingresos del Año</span>
                  <span className="text-base font-extrabold text-emerald-700">
                    +{formatCurrency(yearlyMetrics.yearlyIncome, currencyCode, currencySymbol)}
                  </span>
                </div>
                <div className="p-3 bg-white border border-slate-200/80 rounded-xl text-center shadow-3xs">
                  <span className="text-slate-500 font-semibold text-xs block">Gastos del Año</span>
                ...
                <div className={`p-3 border rounded-xl text-center shadow-3xs ${
                  ...
                  <span className="font-semibold text-xs block text-slate-600">Balance Neto Anual</span>
                ...
                {yearlyMetrics.months.map((m) => {
                  ...
                  return (
                    <div
                      key={m.monthIndex}
                      onClick={() => {
                        onPeriodChange({ ...period, month: m.monthIndex, periodType: 'mes' });
                        setViewMode('grid');
                      }}
                    ...
                    >
                      ...
                      <div className="grid grid-cols-7 gap-0.5 text-[8px] font-bold text-slate-400 text-center uppercase tracking-wider mb-1">
                        <span>L</span><span>M</span><span>M</span><span>J</span><span>V</span><span>S</span><span className="text-rose-500">D</span>
                      </div>
            ...
            <div className="p-4 sm:p-5 space-y-4 max-h-[600px] overflow-y-auto">
              {Object.keys(eventsByDate).length === 0 ? (
                <div className="text-center py-12 text-slate-400">
                  ...
                  <p className="text-sm font-semibold text-slate-600">No hay movimientos registrados para este mes.</p>
                            ...
                            >
                              <div className="flex items-center gap-2 truncate">
                                ...
                                <div className="truncate">
                                  ...
                                  <div className="text-[10px] text-slate-500 truncate">{ev.subtitle || ev.tag}</div>
                                </div>
                              </div>
          ...
          <div className="px-4 py-3 bg-slate-50 border-t border-slate-200/80 flex flex-wrap items-center justify-between gap-3 text-xs">
            <div className="flex flex-wrap items-center gap-3">
              <span className="font-bold text-slate-500 text-[11px]">Colores:</span>
              <div className="flex items-center gap-1.5">
            ...
            <div className="flex items-center gap-3">
              {!isDetailExpanded && (
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 text-xs font-bold flex items-center gap-1.5 transition cursor-pointer shadow-2xs"
              ...
              <div className="text-[11px] font-bold text-slate-500">
                Balance Neto Mes:{' '}
                <strong className={monthlyMetrics.netBalance >= 0 ? 'text-emerald-700' : 'text-rose-700'}>
        ...
        {/* Selected Day Details Panel (4 Cols on Desktop when expanded, Collapsible) */}
        {isDetailExpanded && (
            ...
            <div className="bg-white rounded-2xl border border-slate-200/90 shadow-xs p-4 sm:p-5">
              <div className="flex items-center justify-between pb-3 border-b border-slate-100 mb-3">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    title="Contraer panel (ver almanaque completo)"
                  >
              ...
              <div className="grid grid-cols-2 gap-2 mb-4">
                <div className="p-2.5 bg-emerald-50/70 border border-emerald-100 rounded-xl">
                  <span className="text-[10px] font-bold text-emerald-800 uppercase block">🟢 Ingresos</span>
                ...
                <div className="p-2.5 bg-rose-50/70 border border-rose-100 rounded-xl">
                  <span className="text-[10px] font-bold text-rose-800 uppercase block">🔴 Gastos</span>
                ...
                <div className="p-2.5 bg-amber-50/70 border border-amber-100 rounded-xl">
                  <span className="text-[10px] font-bold text-amber-800 uppercase block">🟡 Por Clasificar</span>
                    ...
                    <div
                      ...
                      className={`p-3 rounded-xl border text-xs transition relative group ${
                        ev.category === 'income'
                          ...
                          : 'bg-blue-50/50 border-blue-200/90 text-blue-950'
                      }`}
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            ...
                            <div className="text-[11px] text-slate-500 mt-0.5 flex items-center gap-1.5 flex-wrap">
                              {ev.tag && (
                                <span className="bg-white/80 border border-slate-200 px-1.5 py-0.2 rounded text-[10px] font-medium">
                                  {ev.tag}
                                </span>
                              )}
                        ...
                        <div className="text-right shrink-0">
                          ...
                          {/* Action buttons if pending expense or transaction */}
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 flex flex-col gap-2">
                ...
                <button
                  ...
                  className="w-full px-3 py-2 bg-amber-50 hover:bg-amber-100 active:bg-amber-200 text-amber-900 border border-amber-300 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition cursor-pointer shadow-2xs"
                >
              ...
              <div className="bg-white rounded-2xl border border-slate-200/90 p-4 shadow-2xs">
                ...
                <div className="grid grid-cols-2 gap-1.5 text-xs">
                  <button
                    ...
                    className="p-2 rounded-xl bg-slate-50 hover:bg-amber-50 hover:text-amber-800 border border-slate-200 transition text-left flex items-center gap-1.5 font-semibold text-slate-700 cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="p-2 rounded-xl bg-slate-50 hover:bg-blue-50 hover:text-blue-800 border border-slate-200 transition text-left flex items-center gap-1.5 font-semibold text-slate-700 cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="p-2 rounded-xl bg-slate-50 hover:bg-rose-50 hover:text-rose-800 border border-slate-200 transition text-left flex items-center gap-1.5 font-semibold text-slate-700 cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="p-2 rounded-xl bg-slate-50 hover:bg-teal-50 hover:text-teal-800 border border-slate-200 transition text-left flex items-center gap-1.5 font-semibold text-slate-700 cursor-pointer"
                  >
